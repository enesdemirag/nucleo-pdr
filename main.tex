...
\SetWatermarkText{\includegraphics[angle=-45]{images/teknofest-watermark.png}}
% watermark icin
\usepackage{hyperref}
% \usepackage{float}
\usepackage{caption}
...
\paragraph{} Takımımızın ROV sistemi yüzeyde bulunan bir kontrol istasyonu, ve ROV'un kendisinden oluşmaktadır. Kontrol istasyonunda 15.6 inçe kadar dizüstü bilgisayar kullanımı desteklenmektedir. Aynı zamanda yüzey kontrol istasyonunda 12VDC 20A kapasiteli ROV'un güç kaynağı bulunmaktadır, güç kaynağı ile ROV güç beslemesi arasında 20A bir sigorta bulunmaktadır. Taşınabilir yüzey istasyonunu prize bağlanması ve ROV'unda yüzey istasyonuna bağlanmasının ardından ROV kullanıma hazır olmaktadır.
...
\paragraph{} Nucleo, gücünü şebekeden gelen 220VAC gerilimi 12VDC'ye dönüştüren bir güç kaynağından alacaktır. Nucleo'nun güç altı motoru ve diğer elektronik komponentlerinin güç gereksinimi göz önüne alındığında 30A taşıyabilecek 14AWG kablo yeterli olacaktır.

\subsubsubsection{Temel Bileşenler}

\paragraph{} 
\subsubsubsection{Sensörler ve Kameralar}
\subsubsubsection{Güç İletimi ve Dağıtımı}
\paragraph{}


The AUV has two main sections(back and front) and some little sections for other elements like torpedo and manipulator. Back section contains our battery, ESC's, microcontrollers and sensor boards. The vehicle is supplied with 4S10P Lithium-ion battery. Our battery also has a custom PCB that watches the state of charge and the state of health of the battery 7/24. The PCB has also 12V, 9V and 5V outputs. There are 8 thrusters and 8 ESC's. 

Front section contains our CPU and cameras. Power transmission and communication are provided between these sections through bulkhead. Vehicle's working principle is simple, yet effective. Microcontrollers collect data from sensors and transmit those data to the NVDIA Xavier and receive commands from it. The communication occurs through TCP/IP protocol. We designed multiple PCB's to make debugging easier and in case of emergency, we wouldn't had to change the whole system.

Under the vehicle, we have our hydrophones and a custom PCB that processes the signals and sends the data to main microcontroller. We used a 480MHz high speed microcontroller for hydrophone circuit and placed it right next to our hydrophones to minimize the environment's effects on the signals. 


\paragraph{} {Bu kısımda aracın elektronik tasarım süreçleri anlatılır. Aracın ön tasarımına ait görsellere (Blok şema, teknik resim, prototip fotoğrafları vb.) yer verilir. Kullanılması planlanan alt bileşenler (Sensörler, kameralar, kablolar, anakart, motor sürücüler, güç kartları vb. ) hakkında bilgi verilir.}
